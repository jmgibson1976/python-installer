...
class TestEnvStubDynaconfPlusLogging:
    """env.py must be created when dynaconf is selected together with logging=True."""

    def test_env_py_created_when_dynaconf_and_logging(self, tmp_path):
        render_stubs(_answers(logging=True, env_parsing="dynaconf"), tmp_path)
        assert (tmp_path / "src" / "my_app" / "env.py").exists()

    def test_env_py_has_get_env_for_dynaconf_plus_logging(self, tmp_path):
        render_stubs(_answers(logging=True, env_parsing="dynaconf"), tmp_path)
