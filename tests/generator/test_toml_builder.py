...
from installer.generator.toml_builder import build_dependencies, build_stdlib_notes, render_toml
...
class TestBuildStdlibNotes:
    def test_sqlite_driver_yields_sqlite3_in_runtime(self):
        runtime, _ = build_stdlib_notes(_answers(db_driver="sqlite"))
        assert "sqlite3" in runtime

    def test_non_stdlib_driver_not_in_runtime(self):
        runtime, _ = build_stdlib_notes(_answers(db_driver="postgresql"))
        assert not runtime

    def test_argparse_cli_yields_argparse_in_runtime(self):
        runtime, _ = build_stdlib_notes(_answers(cli_support="argparse"))
        assert "argparse" in runtime

    def test_unittest_yields_unittest_in_dev(self):
        _, dev = build_stdlib_notes(_answers(testing_frameworks=["unittest"]))
        assert "unittest" in dev

    def test_mock_yields_unittest_mock_in_dev(self):
        _, dev = build_stdlib_notes(_answers(testing_frameworks=["mock"]))
        assert "unittest.mock" in dev

    def test_no_stdlib_choices_returns_empty_lists(self):
        runtime, dev = build_stdlib_notes(
            _answers(db_driver="postgresql", cli_support="typer", testing_frameworks=["pytest"])
        )
        assert runtime == []
        assert dev == []
